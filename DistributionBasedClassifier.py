...
class DistributionBasedClassifier(SentimentClassifier):
    ...
    def __init__(self, lexica=None):
        # Initialize the classifier with lexicon and a tokenizer
        self.lexicon = self.get_lexicon(lexica)
        self.tokenizer = TwitchTokenizer()

    def get_lexicon(self, lexica_to_use: dict):
        # Load lexica (emotes, emojis, VADER) and merge them into a unified lexicon
        # to be used for classification

        # If lexica_to_use is not provided, use all lexica
    ...
    def classify_df(self, df, with_id: bool = False):
        # Classify a DataFrame containing messages
        cs, senti, infer = [], [], []
    ...
    def evaluate(self, twitch_data: pd.DataFrame):
        # Evaluate the classifier's performance on labeled data
