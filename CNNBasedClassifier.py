...
class CNNBasedClassifier(SentimentClassifier):
    ...
    def prepare_message(self, message): # Prepare a message for prediction by converting it into a one-hot encoded list
    ...
    def predict_batch(self, message_list, cnn=None): # Predict the sentiment of a batch of messages
        message_list = []
    ...
    def find_best(self, x_test, y_test): # Find the best model and evaluate it
        print("\nEvaluating...\n")
    ...
    def classify_df(self, df, message_row="message", batch_size=1000):  # Classify a DataFrame containing messages
